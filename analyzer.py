...
def dice(prediction, truth):
    return (
        2.0
        * torch.sum(prediction * truth, dim=[1, 2, 3, 4])
        / (torch.sum((prediction ** 2 + truth ** 2), [1, 2, 3, 4]) + EPS)
    )
...
def dice_2(prediction, truth):
    return (
        2.0
        * torch.sum(prediction * truth, dim=[0, 1, 2])
        / (torch.sum((prediction ** 2 + truth ** 2), [0, 1, 2]) + EPS)
    )
        ...
        if batch_idx in list(range(16)):
            ...
            for i in range(batch_image.shape[0]):
                ...
                grid_sampler = GridSampler(
                    input_tensor, patch_size_test, patch_overlap,
                )
                patch_loader = torch.utils.data.DataLoader(
                    grid_sampler, batch_size=test_batch
                )
            ...
            test_outputs = torch.cat(
                (test_outputs, foreground.cpu().unsqueeze(dim=0)), dim=0
            )
            test_outputs_thres = torch.cat(
                (test_outputs_thres, output_thres.cpu().unsqueeze(dim=0)), dim=0
            )
...
for i in range(len(eval_loader)):
    dice_index = dice_score(
        test_outputs_thres[i, 0], torch.tensor(eval_dataset.data[i]["label"][0])
    )
    print(f"Rat eval volume {i}, Dice score = {dice_index}")
    with open("results/" + sys.argv[2] + ".txt", "w") as f:
        f.write(dice_index.values())
print("\n\n")
...
for i in range(len(eval_loader)):
    print(f"---------Volume {i}---------")
    pred_instances = bc_watershed(
        test_outputs[i].numpy() * 255,
        thres1=0.5,
        thres2=0.8,
        thres3=0.1,
        thres_small=128,
    ...
    get_mAP_3d(
        gt_seg=eval_dataset.data[i]["instances"][0],
        pred_seg=pred_instances,
        predict_heatmap=test_outputs[i][0],
    )

    # %%
    pred_instances_dict = {"pred_instances": pred_instances.astype(np.uint16)}
    hdf5storage.savemat(
        "./" + f"pred_instances_rat_{i}.mat", pred_instances_dict, format="7.3"
    )

    # %%
    semantic_dict = {"pred_semantic": test_outputs_thres[i].numpy().astype(np.uint8)}
    hdf5storage.savemat(
        "./" + f"pred_semantic_rat_{i}.mat", semantic_dict, format="7.3"
    )
    print("\n\n")
